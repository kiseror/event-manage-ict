...
def EventDetails():
    ...
    cursor.execute("CREATE TABLE IF NOT EXISTS event_details (event_name TEXT, event_id TEXT PRIMARY KEY, event_date TEXT, event_time TEXT, event_duration Text, event_description TEXT)")
...
def CreateNewEvent(event_name, event_id, event_date, event_time, event_duration, event_description):
    try:
        conn = sqlite3.connect("event_database.db")
        cursor = conn.cursor()
        cursor.execute("INSERT INTO event_details (event_name, event_id, event_date, event_time, event_duration, event_description) VALUES (?, ?, ?, ?, ?, ?)", (event_name, event_id, event_date, event_time, event_duration, event_description))
        conn.commit()
        conn.close()
        return 'Success'
    except sqlite3.Error as e:
